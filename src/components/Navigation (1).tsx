    ...
    <nav className={`transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-8 py-6 flex items-center justify-between">
        <Link to="/" className="transition-opacity duration-300 hover:opacity-80">
          <img 
            src={(isAboutPage || isPackagesPage || isPricingPage || isSystemsPage || isJournalPage || isContactPage || isLegalPage) ? logoColored : logo} 
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          <Link 
            to="/systems" 
            className={`transition-colors duration-300 ${
              isScrolled || !isHomePage || isSystemsPage
                ? 'text-[#0A203F] hover:text-[#203F4F]' 
                : 'text-white/90 hover:text-white'
            }`}
          >
